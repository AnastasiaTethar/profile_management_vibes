...
const { profileDb, logDB } = require("./config/db");
...
app.get("/api/status", (req, res) => {
  try {
    const profileDbStatus = profileDb.connection.readyState === 1 ? "connected" : "disconnected";
    const logDbStatus = logDB.connection.readyState === 1 ? "connected" : "disconnected";
...
app.get("/api/logs", authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Server error while loading audit logs." });
  }
});
